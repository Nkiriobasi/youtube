...
import { SECTION1, SECTION2, SECTION4, SECTION5, SECTION6 } from "./sidebar.constants";

const Sidebar = () => {
  const pathname = usePathname();

  return (
    <aside className={`z-[2005] w-[240px] fixed right-auto left-0 top-0 bottom-0 transition-all`}>
            ...
            <div className="scrollbox-inner flex-1 basis-[1e-9px]">
              <div className="block relative">
                <div className="border-b border-solid dark:border-[#ffffff33] border-[#eee] p-3 block">
                  {SECTION1.map(({id, href, title, activeSvgElement, inActiveSvgElement}) => {
                    ...
                    return (
                      <div key={id} className="relative w-full block">
                        <Link 
                          ...
                          hover:dark:bg-[#ffffff14] hover:bg-[#eee] rounded-[10px] ${isActive ? 'dark:bg-[#ffffff14] bg-[#eee]' : ''}`}
                        >
                          <div className="px-3 w-full flex flex-row items-center leading-[24px] font-normal text-sm relative">
                            ...
                            <h4 
                              ...
                              text-ellipsis flex-1 ${isActive ? 'font-semibold' : 'font-normal'}`}
                            >
                              {title}
                            </h4>
                          </div>
                  ...
                  <div className="border-t border-solid dark:border-[#ffffff33] border-[#eee] mt-3 pt-3 block">
                    {SECTION2.map(({id, href, title, activeSvgElement, inActiveSvgElement}) => {
                      const isActive = pathname.startsWith(href)
                ...
                <div className="p-3 border-b border-solid dark:border-[#ffffff33] border-[#eee] block">
                  <span>
                    <h3 className="block pt-[6px] px-3 pb-1 dark:text-[#f1f1f1] text-black leading-[2.2rem] font-normal text-base">Explore</h3>
                  </span>

                  <div>
                    {SECTION4.map(({id, href, title, activeSvgElement, inActiveSvgElement}) => {
                      const isActive = pathname.startsWith(href)
                ...
                <div className="p-3 border-b border-solid dark:border-[#ffffff33] border-[#eee] block">
                  <span>
                    <h3 className="block pt-[6px] px-3 pb-1 dark:text-[#f1f1f1] text-black leading-[2.2rem] font-normal text-base">More from Youtube</h3>
                  </span>
                ...
                {/* Misc */}
                <div className="p-3 border-b border-solid dark:border-[#ffffff33] border-[#eee] block">
                  {SECTION6.map(({id, href, title, activeSvgElement, inActiveSvgElement}) => {
                    const isActive = pathname.startsWith(href)
