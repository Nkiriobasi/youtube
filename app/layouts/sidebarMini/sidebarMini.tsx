...
const SidebarMini = (): JSX.Element => {
  ...
  return (
    <aside className='fixed left-0 bottom-0 top-[56px] w-[72px] inline-block bg-inherit z-[2028] px-1'>
        ...
        {sidebarMiniLinks.map(
          ({ id, href, title, activeSvgElement, inActiveSvgElement }) => {
            ...
            return (
              <div
                key={id}
                className='w-full flex justify-center relative bg-inherit'>
                <Link
                  href={href}
                  title={title}
                  className='pt-4 pb-[14px] px-1 outline-none w-full flex flex-col items-center justify-center
